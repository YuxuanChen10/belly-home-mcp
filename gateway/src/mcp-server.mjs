...
const schedule = z.discriminatedUnion("kind", [
  z.object({
    kind: z.literal("once"),
    fireAt: z.string().describe("RFC 3339 timestamp with an explicit UTC offset")
  }),
  z.object({
...
server.registerTool("create_alarm", {
  ...
  try {
    return toolResult(await client.createAlarm({
      label,
...
server.registerTool("list_alarms", {
  ...
  try {
    return toolResult(await client.listAlarms(includeCancelled));
  } catch (error) {
    return failure(error);
  }
});
...
server.registerTool("get_alarm_status", {
  ...
  try {
    return toolResult(await client.getAlarm(alarmId));
  } catch (error) {
    return failure(error);
  }
});
...
server.registerTool("update_alarm", {
  ...
  try {
    return toolResult(await client.updateAlarm(alarmId, {
...
server.registerTool("cancel_alarm", {
  ...
  try {
    return toolResult(await client.cancelAlarm(alarmId));
  } catch (error) {
    return failure(error);
  }
});
