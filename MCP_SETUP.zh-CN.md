...
- MCP HTTP endpoint 对 `2026-07-28` 客户端的 `server/discover` 预探测返回 HTTP 400 与标准 `-32022 UnsupportedProtocolVersion`，并通过 `data.supported` 声明 SDK 1.30.0 实际支持的 legacy 协议版本。客户端应随后降级到 `2025-11-25` 的 `initialize` session；服务端不虚假声明支持完整 stateless 2026 协议。
